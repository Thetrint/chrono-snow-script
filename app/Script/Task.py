...
class StartTask:
    ...
    def start(self, row, handle):
        ...
        execute = True
        time.sleep(1)
        event.persona = Persona(row, handle, mapp)
        for i in range(6):
...
class BasicTask(object):
    ...
    def leave_team(self):
        ...
        if not self.coord('队伍界面', histogram_process=True, threshold=0.7):
            self.key_down_up(event.persona.team)
    ...
    def open_entrance(self, tem_name, laplacian_process, binary_process, threshold):
        ...
        for i in range(2):
            ...
            self.key_down_up(event.persona.knapsack)
